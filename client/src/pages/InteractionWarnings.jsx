...
function toRow(data, a, b) {
  const nameA = data.pair?.drugA ?? a;
  const nameB = data.pair?.drugB ?? b;
...
export default function InteractionWarnings() {
  const { medications, interactions, loading, acknowledgeInteraction } =
    useMedication();
        ...
        {checking ? (
          <div className="flex items-center gap-3 rounded-xl bg-white px-4 py-3 shadow-sm">
            ...
            <span className="text-lg">
              Checking interactions… {progress.done}/{progress.total}
            </span>
          </div>
        ) : (
          <button
            ...
            {allInteractions
              ...
              .map((row) => (
                <InteractionCard
                  ...
                  acknowledged={false}
                  onAcknowledge={
                    row.id ? () => handleAcknowledge(row) : undefined
                  }
                />
              ))}
