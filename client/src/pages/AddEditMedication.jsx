...
// ─── Mock photo results (Step 6 API wiring replaces the setTimeout later) ─────
const mockPhotoResult = {
  ...
  dosageUnit: 'mg',
  form: 'tablet',
  frequencyText: '3x sehari',
  confidence: 'high',
  uncertaintyReason: null,
}

const mockPhotoUncertain = {
  readable: false,
  name: null,
  ...
  frequencyText: null,
  confidence: 'low',
  uncertaintyReason: 'The image was not clear enough to read.',
}

// ─── Options ────────────────────────────────────────────────────────────────
const UNITS = ['mg', 'mcg', 'g', 'ml', 'IU', 'tablet', 'capsule', 'drop', 'puff']
...
export default function AddEditMedication() {
  ...
  const { medications, loading, addMedication, updateMedication } = useMedication()
  ...
  // Captured image, kept for the real API call wired in a later step.
  const [, setPhotoImage] = useState(null)
  ...
  function prefillFromPhoto(data) {
    ...
    setForm((f) => {
      const next = { ...f }
      if (data.name != null) { next.name = data.name; filled.add('name') }
      if (data.activeIngredient != null) { next.genericName = data.activeIngredient; filled.add('genericName') }
      if (data.dosageAmount != null) { next.dosageAmount = String(data.dosageAmount); filled.add('dosageAmount') }
      if (data.dosageUnit != null) { next.dosageUnit = data.dosageUnit; filled.add('dosageUnit') }
  ...
  function handleCaptureComplete({ imageBase64, mimeType, fileName }) {
    setPhotoImage({ imageBase64, mimeType }) // held for the real API call later
    setShowCapture(false)
    setPhotoState('photo_processing')
    // No API yet: mock the read. A filename containing "unclear" exercises the
    // uncertain path; everything else returns the Proris mock.
    const uncertain = (fileName || '').toLowerCase().includes('unclear')
    setTimeout(() => {
      prefillFromPhoto(uncertain ? mockPhotoUncertain : mockPhotoResult)
    }, 2000)
  }
    ...
    <main className="mx-auto max-w-2xl px-4 py-6">
      ...
      {photoState === 'photo_prefilled' && (
        <div className="mb-5 rounded-xl border-l-4 border-[var(--color-primary)] bg-[#EFF6FF] px-4 py-3 text-[var(--color-text)]">
          📷 We read these from your photo — please check and correct anything that's wrong.
        </div>
      )}
      {photoState === 'photo_uncertain' && (
        <div className="mb-5 flex items-start justify-between gap-3 rounded-xl border-l-4 border-[var(--color-caution)] bg-[#FFFBEB] px-4 py-3 text-[var(--color-caution)]">
          <span>📷 We couldn't read this clearly — please enter the details yourself.</span>
          <button
            type="button"
            aria-label="Dismiss"
      ...
      {!isEdit && photoState !== 'photo_processing' && (
        showCapture ? (
          <PhotoCapture
      ...
      >
        {/* Name */}
        <div>
          <label htmlFor="name" className={labelBase}>Medicine name</label>
          <input
            id="name"
            type="text"
            value={form.name}
          ...
          />
          {showError('name') && (
            <p className="mt-1 text-base text-[var(--color-danger)]">{showError('name')}</p>
          )}
        </div>

        {/* Active ingredient */}
        <div>
          <label htmlFor="genericName" className={labelBase}>Active ingredient (optional)</label>
          <input
            id="genericName"
            type="text"
        ...
        <div className="flex flex-col gap-4 sm:flex-row">
          <div className="sm:flex-1">
            <label htmlFor="dosageAmount" className={labelBase}>Dose</label>
            <input
              id="dosageAmount"
              type="number"
              min="0"
            ...
            {showError('dosageAmount') && (
              <p className="mt-1 text-base text-[var(--color-danger)]">{showError('dosageAmount')}</p>
            )}
          </div>
          <div className="sm:w-44">
            <label htmlFor="dosageUnit" className={labelBase}>Unit</label>
            <select
              id="dosageUnit"
            ...
            {showError('dosageUnit') && (
              <p className="mt-1 text-base text-[var(--color-danger)]">{showError('dosageUnit')}</p>
            )}
          </div>
        </div>
        ...
        {/* Frequency */}
        <div>
          <label htmlFor="timesPerDay" className={labelBase}>Times per day</label>
          <input
            id="timesPerDay"
            type="number"
            min="1"
        ...
        <div className="flex flex-col gap-3">
          {form.times.map((t, i) => (
            <div key={i}>
              <label htmlFor={`time-${i}`} className={labelBase}>Dose {i + 1} time</label>
              <input
                id={`time-${i}`}
                type="time"
                value={t}
        ...
        <div className="flex flex-col gap-4">
          <div>
            <label htmlFor="startDate" className={labelBase}>Start date</label>
            <input
              id="startDate"
              type="date"
          ...
          {!form.ongoing && (
            <div>
              <label htmlFor="endDate" className={labelBase}>End date</label>
              <input
                id="endDate"
                type="date"
                value={form.endDate}
              ...
              />
              {showError('endDate') && (
                <p className="mt-1 text-base text-[var(--color-danger)]">{showError('endDate')}</p>
              )}
            </div>
          )}
        </div>
